...
import { updateQuantityPrize, getPrizesData } from "./hc-storage-service.js";
    ...
    hcLuckywheel.init({
      ...
      gotBack: function (data) {
        ...
        if (data == null) {
          ...
          updateQuantityPrize(prizes[getPrizeIndex]);
          Swal.fire({
            title:
              "Bạn đã trúng " +
              data +
              (prizes[getPrizeIndex].type == "cgv"
                ? " có mã " +
                  `<span style="color: red;">${prizes[getPrizeIndex].voucherId[0]}</span>`
                : ""),
            text: "Chụp màn hình lại và điền thông tin nhận quà",
            icon: "success",
            confirmButtonText: "Điền thông nhận quà",
          }).then((result) => {
